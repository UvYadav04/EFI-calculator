...
form1.addEventListener('submit', function (e) {
    ...
    if (point_input.checked == 1) {
        ...
        form2.addEventListener('submit', (e) => {
            ...
            for (let i = charge_no.value; i > 0; i--) {
                ...
                label2.innerText = `ρ${i}:`
                label3.innerText = `ϕ${i}:`
                label4.innerText = `z${i}:`
    ...
    label_refx.innerText = "ρ:"
    label_refy.innerText = "ϕ:"
    label_refz.innerText = "z:"
...
form3.addEventListener('submit', function (e) {
    ...
    let rho = xref.valueAsNumber
    let theta = yref.valueAsNumber
    let z = zref.valueAsNumber

    theta = theta * 3.14 / 180
    console.log(theta)

    let xr = rho * Math.cos(theta)
    let yr = rho * Math.sin(theta)
    let zr = z
    ...
    if (point_input.checked == 1) {
        ...
        const z_cy = document.querySelectorAll('#rz')
        ...
        for (let j = 0; j < size.value; j++) {
            let Theta_x = theta_in[j].valueAsNumber * 3.14 / 180
            console.log(Theta_x)
            x.push((rho_x[j].valueAsNumber * Math.cos(Theta_x)))
        }
        ...
        for (let k = 0; k < size.value; k++) {
            let Theta_y = theta_in[k].valueAsNumber * 3.14 / 180
            y.push((rho_x[k].valueAsNumber * Math.sin(Theta_y)))
        }
    ...
    if (volume_input.checked == 1) {
        ...
        const xv = document.querySelector('#centerx')
        const yv = document.querySelector('#centery')
        const zv = document.querySelector('#centerz')
        ...
        let r = Math.sqrt((xr - xv.value) ** 2 + (yr - yv.value) ** 2 + (zr - zv.value) ** 2)
    ...
    let phi = Math.atan(Ey / Ex)

    let ey = (Ey.toPrecision(3) * Math.cos(phi).toPrecision(3))
    let ex = (Ex.toPrecision(3) * Math.sin(phi).toPrecision(3))
    console.log(`Ey : `, ey)
    console.log(`Ex : `, ex)
    let Y = (ex - ey).toPrecision(3)

    const calculation = document.querySelector('#result')

    calculation.innerText = `${(Ex * Math.cos(phi) + Ey * Math.sin(phi)).toPrecision(3)}aρ + ${Y}aϕ + ${Ez.toPrecision(3)}az `
